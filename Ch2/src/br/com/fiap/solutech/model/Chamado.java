...
public class Chamado {
    ...
    public Chamado( String nomeCompleto, String dataDeNascimento, String rg, String cpf,
        ...
        if(rg.length()>9 || rg.length()<9) {
        	if(rg.length()>9) {
        		throw new ValorInvalido("O rg deve ter no máximo 12 caracteres");
        	}else {
        		throw new ValorInvalido("O rg deve ter 12 caracteres");
        	}
        }
        this.cpf = cpf;
        if(cpf.length()>11 || cpf.length()<11) {
        	if(cpf.length()>11) {        		
        		throw new ValorInvalido("O cpf deve ter no máximo 14 caracteres");
        	}else {
        		throw new ValorInvalido("O cpf deve ter 14 caracteres");
        	}
        }
